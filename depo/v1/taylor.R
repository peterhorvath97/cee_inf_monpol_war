...
mod1 <- lm(r ~ -1 + pi + y_gap, data = data_taylor)


mod2 <- lm(r ~ -1 + lagr + pi + y_gap, data = data_taylor)

mod3 <- lm(r ~ -1 + lagr + pi + y_gap + e_gap, data = data_taylor)
...
panel_plot <- data %>% bind_cols(mod1$fitted.values) %>% 
  ...
  scale_color_manual(breaks = c("Interest", "Simple", 
                     ...
                     values = c("black", "red",
                                "blue", "green",
                                "purple")) +
  ggtitle("Interest rates and Taylor rules - Panel estimation") +
...
data_taylor <- data %>% 
  ...
  mutate(lagr = lag(r)) %>% 
  nest() %>% 
  mutate(mod1 = map(data, ~lm(r-0.5*pi_targ ~ pi + y_gap,
                                    data = .x)),
         mod2 = map(data, ~lm(r-0.8*0.5*pi_targ ~ lagr + pi + y_gap,
                                    data = .x)),
         mod3 = map(data, ~lm(r-0.8*0.5*pi_targ ~ lagr + pi + y_gap + e_gap,
                                    data = .x)),
  )
